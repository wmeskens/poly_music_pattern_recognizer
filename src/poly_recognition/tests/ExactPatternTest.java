...
import poly_recognition.analysers.ExactAnalyser;
import poly_recognition.patterns.ExactPattern;
import poly_recognition.patterns.Pattern;

public class ExactPatternTest {
	private ExactPattern basePat;
	private ExactPattern pat1;
	...
	public void setUp() throws Exception {
		...
		Note note1 = new Note(10.1F,1,1,1,1,11);
		...
		notes1.add(note31);
		Time time1 = new Time(0, 0, 1, notes1);
		...
		memory.add(memory1);
		memory.add(memory1);
		ArrayList<PatternValue> patNotes1 = new ArrayList<PatternValue>();
		patNotes1.add(note11);
		patNotes1.add(note12);
		patNotes1.add(note13);
		patNotes1.add(note14);
		ArrayList<PatternValue> notes2 = new ArrayList<PatternValue>();
		notes2.add(note21);
		notes2.add(note22);
		notes2.add(note23);
		notes2.add(note24);
		pat1 = new ExactPattern(patNotes1,new ArrayList<Occurrence>(),basePat);
		ArrayList<PatternValue> occNotes1 = (ArrayList<PatternValue>)patNotes1.clone();
		ArrayList<PatternValue> occNotes2 = (ArrayList<PatternValue>)notes2.clone();
		Occurrence occurrence1 = new Occurrence(occNotes1);
		Occurrence occurrence2 = new Occurrence(occNotes2);
	...
	public void splitTest() {
		...
		ExactPattern genPat = (ExactPattern)genPats.get(0);
		assertTrue(pat1.getCurrentValue(3)==null);
		assertTrue(genPat.getCurrentValue(0).equals(new Note(4, 58, 58, 1, 1, 4)));
	...
	public void findGeneralPatsTest(){
		assertTrue(basePat.findExtendGenPats(note11).contains(pat1));
	}
	
	@Test
	public void findExtendNotePatsTest(){
		int patSize = pat1.getSize();
		Note extendNote = new Note(10, 59, 59, 1, 1, 10);
		assertTrue(pat1.findExtendPat(extendNote,memory).equals(pat1));
		assertTrue(pat1.getSize()==patSize+1);
		assertTrue(pat1.getCurrentValue(patSize).sameValue(extendNote));
	}
	...
	public void exactResultsTest(){
		...
		ExactAnalyser an = new ExactAnalyser(file, basePat);
		an.analyse();
